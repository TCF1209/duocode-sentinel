...
| 2026-09-25 | (OCR veto on the printed text) | 1.000 | 1.000 | 1.000 | **1.0000** | 100% | the confusion veto also runs on the upper-cased printed text, so a glyph damaged inside a word canonicalisation drops ("P0RT KLANG", "C0., LTD") escalates instead of reporting a discrepancy; escalate-only, never a MATCH. Score not re-run on this machine (no `data/_grader/`): `submission.json` over `bundle_data` is byte-identical to the scored run (md5 `1c08cd21`), every report field identical except `duration_ms`, the adversarial harness identical in all 16 modes; the held-out draws were not re-generated |
