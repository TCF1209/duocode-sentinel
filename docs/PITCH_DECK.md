...
mentor asked for: *why we built it* — a documentation clerk does this
manually, about 4 minutes per SI/BL pair and 20 seconds to triage each email
...
Instruction against the draft Bill of Lading — seven fields, manually. At a
...
unreadable scan, a blank field, the wrong document. That has to be
escalated to a reviewer with the reason, not guessed at. Sentinel does all
four."
...
four numbers beside it — **520 emails · 46 discrepancies · 20 escalated,
...
person uses it"): *run the inbox · open a flagged case · confirm or
override it — correct a single value or attach the amended SI/BL · send
the drafted reply.*
...
pair compared, every case Sentinel can't decide escalated with the reason
attached. Thirteen seconds. And this is how a desk uses it, four steps:
run the inbox, open a flagged case, confirm or override it — correct a
single value or attach the amended SI/BL — then send the drafted reply."
...
is never reported as a discrepancy — it is escalated to a reviewer.
...
structured output) and the rule it obeys: *nothing it returns is accepted
until it is found again in the document.* One measured pair, large:
**168 → 2** *(cases escalated on wording we invented, rules alone vs
...
inbox: 0 classifier calls, 0 extractor calls, 6 scans transcribed for the
reviewer.*
...
with wording we invented, rules alone escalate 168 of 188 cases; with the
model, two — and false discrepancies stay at zero, because nothing the
model says is accepted until we find it again in the source. You'll see
both in the demo: a scan transcribed for the reviewer, and four unknown
labels read on request."
...
datasets, three from seeds we never developed against — 225 planted
discrepancies, exact field set, 80/80 escalations. *Attacked ourselves:* 16 kinds of damage,
...
CGM's SI template — one bug found and fixed. *Engineering:* 733 tests, 0
...
tier on → the pattern group → a discrepancy case with the source line under
every value → the scan case with its transcript and the review panel →
`/compare` with unknown labels, off then on.

**Say, at the discrepancy case:** "Under every value — the line it was read
...
model transcribed the page for the reviewer — seven fields, and it marks
the illegible ones rather than guessing. The case stays escalated; the
...
**On the slide:** five numbered features, left; the scan transcription
capture, right. **1 Re-check on amendment** — the sender amends the SI or
BL, you attach the amended document on the case, the same check runs
again, the previous result stays on record. **2 Scans transcribed for the
reviewer** — an image-only PDF is still escalated, but with the seven
fields already transcribed by the model, marked as evidence, not a
verdict. **3 Shipper history on the field** — correcting a field shows how
often this shipper had a discrepancy on that same field before. **4 The
original, one click away** — every value carries its line and the source
document opens beside it. **5 A reply drafted from the reviewer
decision** — not the Sentinel result it overrode; a person still presses
send. One line under them: *Every flag carries the line it came from; every
escalation carries the reason and what to do about it.*

**Say:** "At this stage everyone meets the brief, so here is what the
others mostly don't have. The sender amends a document — you attach the
amended one on the case, the check runs again, and the previous result
stays on record. A scan is still escalated, but already transcribed by the
model.
...
(9.1% today, all correct) · false discrepancies (0 of 46; 0 across 16 modes) ·
...
words "No discrepancy / Discrepancy / Escalated", the Compare page's
...
sentence); demo becomes run → discrepancy case → `/compare` off/on, 75 seconds.
...
| Number | Source |
|---|---|
| 520 emails · 46 discrepancies · 20 escalated · 454 no discrepancy | `run.py` over `bundle_data`, 24 Sep, after `.gitattributes` (`STATUS.md` session 9); identical to the deployed API |
...
| 1.0000 × 4; 225 planted discrepancies; 80/80 escalations | `SCORING.md` §4.1 (dev 520/46, held-out 520/57, 320/31, 820/91) |
...
| 733 tests, 0 failing (591 passed, 142 skipped without the dataset) | `pytest --junitxml`, 25 Sep, on the final-round working tree (no `data/bundle`) |
