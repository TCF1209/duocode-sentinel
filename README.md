...
| **Breaks** | OCR character confusion (`NANTONG` → `NANT0NG`) makes **151 of 188** dev documents report a defect the shipment does not have (150 of 172 held out). §5.1 states the caveat as plainly: that mode edits the value itself, and no system separates "the scanner misread a digit" from "the document says that" without a second source. Fuzzy value matching would appear to fix it while swallowing real defects (`docs/DECISIONS.md` §D2). |
